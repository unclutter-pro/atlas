#!/usr/bin/env bash
# Lifecycle policy: before the agent runs a shell command.
# Usage: command-advice.sh "<command>"
# Output: advice text for the agent, or nothing. Never blocks.
#
# When a command looks like it polls/waits on an event (loop+sleep, or a long
# standalone sleep), nudge toward the reminder CLI. Atlas is event-driven by
# design ("no polling") — this keeps that contract.
set -u
